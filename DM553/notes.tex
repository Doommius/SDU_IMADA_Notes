...
\vspace{5mm}
\begin{figure}
\includegraphics[scale=•]{notes_2.12.png}
\caption{\textbf{Figure 2.12} Decreasing external path length}
\end{figure}
\vspace{5mm}
\textbf{Lemma 2.9} The minimum epl for 2 trees with l leaves is $l \lfloor{log \ l}\rfloor+ 2(l-2^{\lfloorlog \ l\rfloor})$
...
\begin{equation}
\frac{l \lfloor log \ l \rfloor +2(l-2^{\lfloor log \ l \rfllor})}{l} = \lfloor log \ l \rfloor + \epsilon
\end{equation}
where $0 \leq \epsilon < 1 $ since $ l-2^{\lfloor log \ l \rfllor}$ is always less than l/2
\end{proof}
\vspace{5mm}
